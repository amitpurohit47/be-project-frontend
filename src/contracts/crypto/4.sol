...
contract CryptoWillContract {
    ...
    UserInfo[] public userInfoRepository;
    mapping(address => UserInfo) public addToUserInfoRepo;
    ...
    function getUserInfo(address userAddress) public view returns(UserInfo memory) {
        return addToUserInfoRepo[userAddress];
    }

    function getUserInfoRepository() public view returns(UserInfo[] memory){
        return userInfoRepository;
    }

    function addOfficer(address officerAddressArg, string memory officerNameArg, string memory officerAdharArg) public restrictedContractOwner {
        bool flag = false;
        for(uint i = 0; i < claimOfficers.length; i++) {
    ...
    function addWill(
        ...
        string[] memory contractCreator,
        string[][] memory nomineeInfo
    ) public payable {
        ...
        UserInfo memory userInfo = UserInfo({
            ...
            email: contractCreator[1],
            name: contractCreator[2],
            adharNo: contractCreator[3]
        });
        userInfoRepository.push(userInfo);
        addToUserInfoRepo[msg.sender] = userInfo;

        uint rows = nomineeAddressArg.length;
        for(uint i = 0;i < rows; i++) {
            UserInfo memory userInfo2 = UserInfo({
                userAddress: nomineeAddressArg[i],
                email: nomineeInfo[i][1],
                name: nomineeInfo[i][2],
                adharNo: nomineeInfo[i][3]
            });
            userInfoRepository.push(userInfo2);
            addToUserInfoRepo[nomineeAddressArg[i]] = userInfo2;
        }
